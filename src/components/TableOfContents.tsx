            ...
            {headings.map((heading) => (
              ...
              >
                <a
                  href={`#${heading.id}`}
                  className={cn(
                    'block hover:text-orange-600 dark:hover:text-orange-400 text-xs truncate group relative',
                    activeId === heading.id ? 'text-orange-600 dark:text-orange-400 font-medium' : 'text-gray-700 dark:text-gray-300'
                  )}
                  onClick={(e) => {
                    e.preventDefault();
