    ...
    <div className="bg-[#F8FAFC] dark:bg-[#151B28] rounded-lg p-3 h-full transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 h-[calc(100%-3rem)]">
        {projects.map((project) => (
          <div
            key={project.id}
            className="bg-gray-50 dark:bg-gradient-to-br dark:from-[#1E1E2E] dark:to-[#2D2D44] rounded-lg p-3 flex flex-col justify-between border border-gray-200 dark:border-gray-800/50 hover:border-orange-500/50 dark:hover:border-orange-500/50 transition-colors"
          >
            <div>
              ...
              {project.topics && project.topics.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {project.topics.map((tech, i) => (
                    <span
                      key={i}
                      className="flex items-center gap-1 text-xs px-2 py-1 rounded-full bg-orange-500/10 text-orange-600 dark:text-orange-400"
                    >
                      {tech}
                    </span>
                  ))}
